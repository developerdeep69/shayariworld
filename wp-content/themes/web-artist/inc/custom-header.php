 ...
 *
 * @uses web_artist_header_style()
 */
function web_artist_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'web_artist_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'web_artist_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'web-artist' ),
		),
	) );
}
add_action( 'after_setup_theme', 'web_artist_custom_header_setup' );

if ( ! function_exists( 'web_artist_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see web_artist_custom_header_setup().
	 */
	function web_artist_header_style() {
		...
		wp_add_inline_style( 'web-artist-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'web_artist_header_style', 10 );